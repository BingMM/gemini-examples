...
#%% Generate FAC pattern

def fac_input(t, mlon, mlat, duration=200, sigmat=20, centerlat=65, mlat_tweak=0, centerlon=105, width=90, 
    ...
    lat_part = lat_fac(mlat, centerlat=centerlat, mlat_tweak=mlat_tweak, L=L, scaling=scaling)
    
    fac = t_part * lon_part * lat_part
    
    return fac

#%% Generate the latitudinal profile for fac_input()

def lat_fac(mlat, centerlat=65, mlat_tweak=0, L = 7628.888, scaling = 10):
    '''
    ...
    if type(mlat) == int or type(mlat) == float:
        mlat = np.array([mlat])
    x = mlat_to_latdist(mlat, mlat0=centerlat+mlat_tweak)
    B_analytical = np.zeros(x.shape)
    current_analytical = np.zeros(x.shape)
    mu0 = 4*np.pi*10**(-7)
    
    # Read coeficient file:
    df = pd.read_hdf('/home/bing/BCSS-DAG Dropbox/Michael Madelaire/work/code/repos/ISSI_506/data/fac_input_coefs.h5')
    N = df.shape[0] - 1
    ...
    g = np.exp(-(mlat-centerlat)**4/(2*45**2)) # Function that will make the ends go to 0 for the current
    current_analytical = current_analytical * g
    
    return current_analytical

#%% Generate the longitudinal profile for fac_input()
...
#%% Generate the temporal profile for fac_input()
...
#%% Example of how to use fac_input()
...
def example():
    ...
    _times = np.arange(0, duration, 10) #temporal locations to evaluare for FAC [minuted]
    ...
    fac = fac_input(times, mlons, mlats, centerlon=centerlon, width=width, 
                    sigmat=sigmat, scaling=scaling) # [A/m2]

    # Some plotting
    clim=4e-6 #A/m2
    tind = 10 # time index to show
    plt.figure()
    plt.pcolormesh(mlons[tind,:,:],mlats[tind,:,:],fac[tind,:,:], cmap='bwr', 
                   vmin=-clim, vmax=clim)
    plt.xlabel('mlon [deg]')
    plt.ylabel('mlat [deg]')
